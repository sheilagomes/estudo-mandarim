...
const perguntas = [["Como você se chama", "nǐ jiào shénme míngzì", "你叫什么名字"], ["Seu pai é professor", "nǐ de bàba shì lǎoshī ma", "你的爸爸是老师吗"], ["Quem é seu pai", "nǐ de bàba shì shuí", "你的爸爸是谁"], ["Ela é amiga de quem", "tā shì shuí de péngyou", "他是谁的朋友"], ["Você está bem", "nǐ hǎo ma", "你好吗"]]
const frases  = [["Eu estou bem", "wǒ hěn hǎo", "我很好"], ["Como você se chama", "nǐ jiào shénme míngzì", "你叫什么名字"], ["Seu pai é professor", "nǐ de bàba shì lǎoshī ma", "你的爸爸是老师吗"], ["Quem é seu pai", "nǐ de bàba shì shuí", "你的爸爸是谁"], ["Ela é amiga de quem", "tā shì shuí de péngyou", "他是谁的朋友"], ["Você está bem", "nǐ hǎo ma", "你好吗"]]
...
function criaHanzi() {
	...
	document.querySelector(".frase-traduzida").innerHTML = `${frases[fraseInicial][2]}`
    for(let j = 0; j < 12; j++) {
    ...
    for(let k = 0; k < frases[fraseInicial][2].length; k++) {
    	while (!(mostraHanzi.includes(frases[fraseInicial][2][k]))) {
    		mostraHanzi[sorteio(0, mostraHanzi.length-1)] =(frases[fraseInicial][2][k])
    	}
    }
    for(let l = 0; l < 12; l++) {
    	document.querySelector(`.frase-hanzi-${l}`).innerHTML = `${mostraHanzi[l]}`
    }
}
...
for (let i = montagemFrase.length - 1; i >= 0; i--) {
	montagemFrase[i].addEventListener("click", function() {
		fraseTraduzidaFinal.push(`${montagemFrase[i].innerHTML}`)
		document.querySelector(".frase-traduzida").innerHTML = `${fraseTraduzidaFinal.toString().replace(/,/g, "")}` 
	})
}